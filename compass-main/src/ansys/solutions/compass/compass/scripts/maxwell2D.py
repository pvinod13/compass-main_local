...
from ansys.solutions.compass.compass.scripts.utils import (
    find_n_largest,
    find_elements,
...
def create_cs_magnets(mod2D, pm_id, cs_name, point_direction):
    """
    ...
    )  # apply method get_edge_midpoint to all elements of list
    # longest_edge_list

    rad = lambda x: math.sqrt(x[0] * x[0] + x[1] * x[1] + x[2] * x[2])
...
def generate_geometry(
    geom_params, aedt_version, project_name, design_name, setup_name, solution_type, working_directory
):
    """
    Usage
    -----
    Generate a sketch of the 2D geometry model of the electrical motor.
    Parameters
    ----------
    geom_params [dict]       : geometrical parameters
    aedt_version [str]       : AEDT version
    project_name [str]       : name of the AEDT project
    design_name [str]        : name of the AEDT design
    setup_name [str]         : setup name
    solution_type [str]      : solution type
    working_directory [Path] : working directory
    Returns
    -------
    results [dict]           : path to pyaedt working folder
    """
    ...
    M2D.save_project(os.path.join(working_directory, project_name + ".aedt"))

    ##########################################################
    # Create mod2D to access M2D.modeler easily
    mod2D = M2D.modeler
    # mod2D.delete() # Comment file line as it return a waring in stderr causing SAF to exit with error
    ...
    create_cs_magnets(mod2D, IPM1_id, "CS_" + IPM1_id.name, "outer")
    create_cs_magnets(mod2D, OPM1_id, "CS_" + OPM1_id.name, "outer")

    #####################################################################################
    # Move PM_O : Webapp PoC feature
    # model is built on a L of 2.280129529mm, so let's move the model of a distance d_move_PM_O = L-2.280129529mm
    d_move_PM_O = float(M2D["L_Drl_PM_O"][:-2]) - 2.280129529
    M2D["d_move_PM_O"] = str(d_move_PM_O) + "mm"

    if d_move_PM_O != 0:
        id_PM_O = mod2D.get_objects_w_string("PM_O", case_sensitive=True)
        vector_move_PM_O = [
            "-d_move_PM_O*cos((360deg/SymmetryFactor/2))",
            "-d_move_PM_O*sin((360deg/SymmetryFactor/2))",
            0,
        ]

        mod2D.move(id_PM_O, vector_move_PM_O)
    ...
    void_small_1_id = mod2D.create_circle(
        position=[62, 0, 0], radius="DiaSmallHoles/2", num_sides=0, name="void1", matname="vacuum"
    )
    ...
    if d_move_PM_O != 0:
        id_slot_OM1 = mod2D.get_objects_w_string("slot_OM1", case_sensitive=True)
        vector_move_PM_O = [
            "-d_move_PM_O*cos((360deg/SymmetryFactor/2))",
            "-d_move_PM_O*sin((360deg/SymmetryFactor/2))",
            0,
        ...
        mod2D.move(id_slot_OM1, vector_move_PM_O)

    M2D.modeler.duplicate_and_mirror(
        [slot_IM_id, slot_OM_id],
        position=[0, 0, 0],
        vector=["cos((360deg/SymmetryFactor/2)+90deg)", "sin((360deg/SymmetryFactor/2)+90deg)", 0],
        is_3d_comp=True,
    )
    ...
    id_bc_2 = mod2D.get_edgeid_from_position(
        [pos_1 + "*cos((360deg/SymmetryFactor))", pos_1 + "*sin((360deg/SymmetryFactor))", 0], obj_name="Region"
    )
    ...
    id_bc_az = mod2D.get_edgeid_from_position(
        [pos_2 + "*cos((360deg/SymmetryFactor/2))", pos_2 + "*sin((360deg/SymmetryFactor)/2)", 0], obj_name="Region"
    )
    ...
    model.plot(os.path.join(working_directory, "Geometry.jpg"))

    M2D.export_3d_model("my_geom", working_directory, ".sat")

    ###############################################
    # Results
    results = {}
    results["working_directory"] = working_directory
...
def run_maxwell_solve(aedt_version, project_name, design_name, setup_name, solution_type, working_directory):
    """Run Maxwell to solve the 2D transient electromagnetics problem."""

    # Load AEDT model
    M2D = Maxwell2d(
        projectname=os.path.join(working_directory, project_name + ".aedt"),
        designname=design_name,
        solution_type=solution_type,
        specified_version=aedt_version,
        non_graphical=True,
    ...
    output_vars = {
        ...
        "Pm_loss": "P_at_1000*(MachineRPM/(1000*1rpm))^2.25",
        "Efficiency": "(slidingmean(OutputPower,StopTime/2) - slidingmean(Pm_loss,StopTime/2)) / (slidingmean("
        "OutputPower,StopTime/2) + slidingmean(ArmatureOhmicLoss_DC,StopTime/2) + slidingmean("
        "CoreLoss,StopTime/2) + slidingmean(SolidLoss,StopTime/2))*100",
    }
    ...
    post_params = {"Moving1.Torque": "TorquePlots", "Efficiency": "Efficiency"}
    #################################################################################
    ...
    faces_reg = M2D.modeler.get_object_faces("Rotor")  # Region
    ...
    # M2D.save_project()
    aedt_results = post_process(M2D, os.path.join(working_directory, project_name + ".pyaedt", design_name))
...
def post_process(M2D, working_directory):
    """Post-process Maxwell solve."""

    results = {}

    # Generate reports
    # first we write csv files
    # TODO: check why it's failing in GetSweeps() in export_results()
    M2D.export_results(analyze=False, export_folder=None)

    # Collect results
    csv_files = get_file_by_extension(working_directory, ".csv")
    to_key = {}
    to_key["efficiency"] = "efficiency"
    to_key["CoreLoss(Stator) [mW]"] = "stator_loss_avg"
    to_key["CoreLoss(Rotor) [mW]"] = "rotor_loss_avg"
    to_key["SolidLoss(PM_I1) [mW]"] = "pm_i1_loss_avg"
    to_key["SolidLoss(PM_I1_1) [mW]"] = "pm_i2_loss_avg"
    to_key["SolidLoss(PM_O1) [mW]"] = "pm_o1_loss_avg"
    to_key["SolidLoss(PM_O1_1) [mW]"] = "pm_o2_loss_avg"
    to_key["StrandedLoss [mW]"] = "stranded_loss_avg"
    to_key["Moving1.Torque [NewtonMeter]"] = "torque_avg"

    mW_to_W = 1.0 / 1000.0
    coeff = mW_to_W

    # then we need to extract average values stored in OutputLosses
    for csv_file_name in csv_files:
        if "OutputLosses" in csv_file_name:
            file_path = os.path.join(working_directory, csv_file_name)
            break
    df = pd.read_csv(file_path)
    headers = []
    for header in df.columns:
        headers.append(header.replace("\n", "").split(" - Airgap")[0].replace("[]", ""))
    df.columns = headers
    for col in df.columns:
        if not col.startswith("Time"):
            results[to_key[col]] = "{:.2f}".format(df[col].mean() * coeff)

    # mean value of Torque
    for csv_file_name in csv_files:
        if "TorquePlot" in csv_file_name:
            file_path = os.path.join(working_directory, csv_file_name)
            break
    df = pd.read_csv(file_path)
    headers = []
    ...
    for col in df.columns:
        if not col.startswith("Time"):
            results[to_key[col]] = "{:.2f}".format(df[col].mean())

    # and get last value of Efficiency
    for csv_file_name in csv_files:
        if "Efficiency" in csv_file_name:
            file_path = os.path.join(working_directory, csv_file_name)
            break
    df = pd.read_csv(file_path)
    results["efficiency"] = "{:.2f}".format(df.tail(1).values[0][1])

    # Debug
    # import json
    # with open("results.json", "w") as outfile:
    #     json.dump(to_key, outfile)

    return results


def post_process_result(file_path):

    try:
        df = pd.read_csv(file_path)
        if math.isnan(df.iloc[0][1]):
...
# Debug
# working_directory = r"C:\Users\iazehaf\Documents\z_debug"
# geom_params = {
#     "DiaGap": "132mm",
#     "DiaStatorYoke": "198mm",
#     "DiaStatorInner": "132mm",
...
#     "L_Drl_PM_O": "2.280129529mm",
# }
# generate_geometry(geom_params, "2022.1", "Example", "Sinusoidal", "MySetupAuto", "TransientXY", working_directory)
# print("-----------------------------------------------------")
# run_maxwell_solve("2022.1", "Example", "Sinusoidal", "MySetupAuto", "TransientXY", working_directory)
